...
def emm_win(game: Blackjack, state: BJState, depth_limit: int = 6) -> Tuple[float, Optional[str]]:
...
def policy_expecti_win(game: Blackjack, state: BJState, depth=6) -> str:
    ...
    _, a = emm_win(game, state, depth_limit=depth)
    return a or A_STAND
...
def run_comparison(rounds=10, iters=2500, depth=6, decks=4, starting_chips=1000, base_bet=50, *,
    ...
    for rnd in range(1, rounds+1):
        ...
        s3 = make_state()  # EMM-Win  (restricted: no insurance, no double)
        ...
        show("EMM-Win", s3, a3, f3, d3c, r3)

    print("\n           ==== Final Chip Stacks ====")
    for tag in ('MCTS-Profit','MCTS-Win','EMM-Win'):
