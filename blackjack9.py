...
# Modified to optionally return value estimates for printing.
...
def mcts_core(game: Blackjack,
              ...
              *,
              debug: bool = False,
              debug_every: int = 500,
              reward_metric: str = "ev",
              return_value: bool = False):
    """
    If return_value=False (default): returns chosen action (str).
    If return_value=True: returns (chosen_action, best_avg, per_action_avgs) where
      - best_avg is the maximum average return among root children,
      - per_action_avgs maps action -> avg_return.
    """
    ...
    def print_children_stats(prefix: str):
        ...
        for action, child in root_node.children.items():
            avg = (child.total_reward/child.visit_count) if child.visit_count>0 else 0.0
            parts.append(f"{action}:N={child.visit_count},avg={avg:.3f}")
        print(f"{prefix} | root.N={root_node.visit_count} | " + " | ".join(parts))
    ...
    if not root_node.children:
        ...
        return (A_STAND, 0.0, {}) if return_value else A_STAND

    # Compute per-action average returns at root
    per_action_avgs = {
        a: (child.total_reward / child.visit_count)
    ...
    best_avg = max(per_action_avgs.values()) if per_action_avgs else 0.0

    if debug:
        print_children_stats("[MCTS] final")
        print(f"[MCTS] chosen action: {chosen} | best_avg={best_avg:.4f} ({reward_metric})")

    return (chosen, best_avg, per_action_avgs) if return_value else chosen
...
# --- Helpers that also return optimal expected utility at the root ---

def mcts_profit_value(game: Blackjack, root_state: BlackjackState, iteration_budget: int, *,
                      debug: bool = False, debug_every: int = 500):
    rng = random.Random(secrets.randbits(64))
    action, best_avg, per_action = mcts_core(
        game, root_state, iteration_budget,
        reward_rollout_fn=rollout_random_ev,
        exploration_constant=math.sqrt(2),
        rng=rng, debug=debug, debug_every=debug_every,
        reward_metric="ev", return_value=True
    )
    return action, best_avg, per_action  # EV in chips

def mcts_win_value(game: Blackjack, root_state: BlackjackState, iteration_budget: int, *,
                   debug: bool = False, debug_every: int = 500):
    rng = random.Random(secrets.randbits(64))
    action, best_avg, per_action = mcts_core(
        game, root_state, iteration_budget,
        ...
        exploration_constant=math.sqrt(2),
        rng=rng, debug=debug, debug_every=debug_every,
        reward_metric="win", return_value=True
    )
    return action, best_avg, per_action  # expected win utility ∈ [-1, 1]
...
def run_comparison(rounds=10, iteration_budget=2500, depth=6, decks=4, starting_chips=1000, base_bet=50, *,
    ...
    for round_index in range(1, rounds+1):
        ...
        # --- Print optimal expected utility at the root for MCTS agents ---
        act_p, best_ev, per_p = mcts_profit_value(game, start_profit, iteration_budget,
                                                  debug=False, debug_every=debug_every)
        print(f"[Root EV]  MCTS-Profit: best_avg={best_ev:+.4f} with action {act_p} | per_action={ {k: round(v,4) for k,v in per_p.items()} }")

        act_w, best_win, per_w = mcts_win_value(game, start_win_mcts, iteration_budget,
                                                debug=False, debug_every=debug_every)
        print(f"[Root Win] MCTS-Win:    best_avg={best_win:+.4f} with action {act_w} | per_action={ {k: round(v,4) for k,v in per_w.items()} }")

        # --- Now actually play the hands (unchanged policies) ---
