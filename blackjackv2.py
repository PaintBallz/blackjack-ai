...
def run_comparison(rounds=10, iters=2500, depth=6, decks=4, seed=None):
    # remove the old: random.seed(seed)
    ...
    for rnd in range(1, rounds+1):
        # NEW: fresh randomness each round
        random.seed(secrets.randbits(64))

        # fresh initial deal (randomized this round)
        init = game.deal_initial()
        ...
        # Round summary
        pv1,_ = hand_value(init.player_cards)
        dv1,_ = hand_value(init.dealer_cards)
        print(f"\n=== Round {rnd} ===")
        print(f"Initial deal -> Player {init.player_cards} ({pv1}) | Dealer {init.dealer_cards[0]} + [hole]")
        print(f"MCTS-Profit    actions: {a1}  -> result EV={game.utility_ev(f1):+0.2f}, final={f1.player_cards}/{f1.dealer_cards}")
        print(f"MCTS-Win       actions: {a2}  -> result EV={game.utility_ev(f2):+0.2f}, final={f2.player_cards}/{f2.dealer_cards}")
        print(f"Expecti-Win    actions: {a3}  -> result EV={game.utility_ev(f3):+0.2f}, final={f3.player_cards}/{f3.dealer_cards}")

    # Totals
    print("\n==== All Winn / Loss Totals ====")
...
if __name__ == "__main__":
    ...
    run_comparison(rounds=100, iters=3000, depth=6, decks=4, seed=34)
